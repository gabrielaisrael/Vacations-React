...
export default function Login() {
  ...
  function loginUser() {
    fetch("http://localhost:1003/users/login", {
      method: "post",
    ...
    <div id="login">
      ...
      <div className="login_div">
        ...
        <form className={classes.root} noValidate autoComplete="off">
          ...
          <br />
          <NavLink to="/register">
            <h3 id="sign_up_link"> Don't have an account? Sign Up!</h3>{" "}
          </NavLink>
        </form>
      </div>
      <br />
      <img
        className="israel_images"
        src="https://media-exp1.licdn.com/dms/image/C4D1BAQHPeavO_OTEqQ/company-background_10000/0?e=2159024400&v=beta&t=wQC__vxLPIXauoLPQP4cHYVk9nw-9ZiTjNPuh7FgkyM"
        alt="israel-images"
      />

      {isLogged && isAdmin ? (
        <Navigate to="/vacations/admin" />
      ) : (
        <Navigate to="/" />
      )}

      {!isAdmin && isLogged ? <Navigate to="/home" /> : null}
    </div>
  );
}
